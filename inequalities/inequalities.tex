...
\maketitle
\section*{Disclaimer}
These notes are a compact compilation of inequalities encountered during Olympiad preparation. They are intended for quick revision and as a reference to some of the most useful and common inequalities in Olympiad problem solving, not as a complete learning resource. For proofs and further inequalities, see
\begin{center}
...
For $a_1,a_2,\ldots,a_n>0$ and nonzero real numbers $k_1>k_2$,
\[
...
\]
Equality iff the vectors $(a_1,a_2,\ldots,a_n)$ and $(b_1,b_2,\ldots,b_n)$ are linearly dependent.
...
For real $a_i,b_i$ and $p,q>1$ with $\frac{1}{p}+\frac{1}{q}=1$,
\[
    \sum_{i=1}^n |a_ib_i|
    \leq
...
\]
be nonincreasing tuples of nonnegative real numbers. We say that $\alpha$ majorizes $\beta$, written $\alpha\succ\beta$, if
\[
    \sum_{i=1}^k\alpha_i\geq\sum_{i=1}^k\beta_i
    \quad (1\leq k<n),
    \qquad
    \sum_{i=1}^n\alpha_i=\sum_{i=1}^n\beta_i.
\]
If $x_1,x_2,\ldots,x_n\geq 0$ and $\alpha\succ\beta$, then
\[
    \sum_{\mathrm{sym}}x_1^{\alpha_1}x_2^{\alpha_2}\cdots x_n^{\alpha_n}
    \geq
    \sum_{\mathrm{sym}}x_1^{\beta_1}x_2^{\beta_2}\cdots x_n^{\beta_n}.
\]
For positive $x_i$ and $\alpha\neq\beta$, equality iff
\[
    x_1=x_2=\cdots=x_n.
\]
